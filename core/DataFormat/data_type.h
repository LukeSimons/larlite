...
namespace larlite {
  ...
  template<> data::DataType_t data_type<wrapper<std::vector<recob::Hit> > > ();     // LARSOFT DATA PRODUCTS
  template<> data::DataType_t data_type<wrapper<std::vector<recob::Cluster> > > (); // LARSOFT DATA PRODUCTS
 }
#endif
